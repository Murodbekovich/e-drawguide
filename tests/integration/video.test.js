...
describe('Video Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        const res = await request(app).post('/api/v1/mobile/auth/admin/login').send({ login: 'super_admin', password: 'Admin@12345' });
        adminToken = res.body.access_token;
    });
    test('Admin yangi video qo\'sha olishi kerak', async () => {
        const res = await request(app)
            .post('/api/v1/admin/videos')
            .set('Authorization', `Bearer ${adminToken}`)
            .send({ title: 'Test Video', video_url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ' });
