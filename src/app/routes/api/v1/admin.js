...
router.post('/library', auditLogger('Library'), upload.fields([{ name: 'book_file', maxCount: 1 }, { name: 'cover_file', maxCount: 1 }]), checkFileSignature(['pdf', 'jpg', 'png', 'jpeg']), validate(createLibrarySchema), adminLibraryController.create);
router.delete('/library/:id', auditLogger('Library'), adminLibraryController.delete);
router.post('/videos', auditLogger('Video'), upload.single('thumbnail_file'), checkFileSignature(['jpg', 'png', 'jpeg']), validate(createVideoSchema), adminVideoController.create);
...
router.post('/quizzes/:id/questions', auditLogger('Quiz'), validate(addQuestionSchema), quizController.addQuestion);
