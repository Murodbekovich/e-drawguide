...
class QuizService {
    ...
    async getQuizById(id, userRole) {
        ...
        const quiz = await Quiz.findByPk(id, { include: [{ model: Question.scope(questionScope), as: 'questions' }] });
    ...
    submitQuiz = transactional(async (userId, quizId, userAnswers, lang = 'uz', transaction) => {
        ...
        if (!correctAnswersMap) {
            const questions = await Question.scope('withAnswer').findAll({ where: { quiz_id: quizId }, attributes: ['id', 'correct_answer'], transaction });
            if (questions.length === 0) throw new AppError(t('quiz.no_questions', lang), 404);
            correctAnswersMap = {};
            questions.forEach(q => { correctAnswersMap[q.id] = String(q.correct_answer).trim().toUpperCase(); });
        ...
        userAnswers.forEach(ans => {
            if (!processedQuestions.has(ans.question_id)) {
                ...
                if (correctAnswer && correctAnswer === studentAnswer) score++;
                processedQuestions.add(ans.question_id);
            }
        });

        const result = await Result.create({ user_id: userId, quiz_id: quizId, score: score, total_questions: totalQuestions }, { transaction });
        return { result_id: result.id, correct: score, total: totalQuestions, percentage: totalQuestions > 0 ? Math.round((score / totalQuestions) * 100) : 0, submitted_at: result.created_at };
    });
}
